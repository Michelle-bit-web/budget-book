...
class Balancebook{
  ...
  _remove_input(timestamp){
    let start_index;
    for (let i = 0; i < this._inputs.length; i++) {
      if(this._inputs[i].get("timestamp") === parseInt(timestamp)){
        start_index = i;
        break;
      };
    }
  ...
  _add_event_recycle_input(list_node){
    list_node.querySelector(".entfernen-button").addEventListener("click", e => {
      ...
      this._remove_input(timestamp);
    });
  }
  _show_inputs() {
    document.querySelectorAll(".monatsliste ul").forEach((list) => list.remove());
    let input_list = document.createElement("ul");
    this._inputs.forEach((input) => input_list.insertAdjacentElement("beforeend", this._generate_html_input(input)));
  ...
  _show_sum() {
    ...
    document.querySelector("body").insertAdjacentElement("beforeend", this._generate_html_sum());
  }
};
